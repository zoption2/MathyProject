        ...
        private async UniTask<ITaskController> CreateTaskInternal(ScriptableTask taskSettings, Transform viewParent)
        {
            ...
            switch (taskSettings.TaskType)
            {
                ...
                case TaskType.CountTo10Frames:
                    model = new FramesCountToTenTaskModel(taskSettings);
                    controller = container.Resolve<FramesCountToTenTaskController>();
                    view = await refsHolder.TaskViewProvider.InstantiateFromReference<IFramesCountToTenTaskView>(TaskType.CountTo10Frames, viewParent);
                    await controller.Init(model, view);
                    return controller;
                ...
                case TaskType.CountToTensAndOnes:
                    ...
                    controller = container.Resolve<FramesCountTensAndOnesTaskController>();
                    view = await refsHolder.TaskViewProvider.InstantiateFromReference<IFramesCountTensAndOnesTaskView>(TaskType.CountToTensAndOnes, viewParent);
                    await controller.Init(model, view);
                    return controller;
